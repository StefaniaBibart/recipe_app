...
export class DataSyncService {
  ...
  private progress = signal<SyncProgress>({ currentOperation: '', operations: [] });
  ...
  async syncAll(): Promise<void> {
    try {
      ...
      localStorage.setItem(this.storageKeys.ingredients, JSON.stringify(ingredients));
  ...
  private async fetchMealsByIngredient(ingredient: string): Promise<MealPreview[]> {
    const response = await firstValueFrom(
      this.http.get<{meals: MealPreview[]}>(`${this.apiUrl}/filter.php?i=${encodeURIComponent(ingredient)}`)
    );
    return response.meals || [];
  }
  ...
  private async fetchAndStoreMealDetails(mealId: string): Promise<MealDetail> {
    ...
    const response = await this.rateLimitedRequest(
      firstValueFrom(
        this.http.get<{meals: MealDetail[]}>(`${this.apiUrl}/lookup.php?i=${mealId}`)
      ),
  ...
  private async rateLimitedRequest<T>(request: Promise<T>, operation: string, skipRateLimit = false): Promise<T> {
  ...
  private updateDataCounts() {
    ...
    const areas = JSON.parse(localStorage.getItem(this.storageKeys.areas) || '[]');
    const ingredients = JSON.parse(localStorage.getItem(this.storageKeys.ingredients) || '[]');
    const meals = Object.keys(JSON.parse(localStorage.getItem(this.storageKeys.meals) || '{}')).length;
